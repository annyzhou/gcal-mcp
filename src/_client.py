...
"""Sample MCP client demonstrating OAuth browser flow for Google Calendar.
...
async def run_with_runner() -> None:
    ...
    result = await with_oauth_retry(
        lambda: runner.run(
            input="List my calendars and show events for today",
            model="openai/gpt-4.1",
            mcp_servers=["your-org/gcal-mcp"],  # Replace with your server slug
        )
    )
    ...
    async def do_request():
        return await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "List my calendars and show events for today",
                }
            ],
            mcp_servers=["your-org/gcal-mcp"],  # Replace with your server slug
        )
